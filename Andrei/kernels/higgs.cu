...
__global__ void Higgs5x256dMatVec(
  const uint4* __restrict__ A,
  const uint4* __restrict__ B,
        uint4* __restrict__ C,
  const half* __restrict__ scales,
  ...
  int b_gl_rd = 0;
  int a_gl_rd_8 = a_gl_stride_8 * c_gl_wr + (threadIdx.x % threads_in_wave) * (scales_size / threads_in_wave) / group_size;
  const int zeroth_offset = (((threadIdx.x % threads_in_wave) % 5) * 2) % 5;
  ...
  while (iters--) {
    ...
    if (pred && a_gl_rd_8 < a_gl_end_8) {
      ...
      #pragma unroll
      for (int j = 0; j < 8; j++) {
        dec[5 * j + 0] = __ldca(((half*)HIGGS_5_256 + 5 * enc[j] + 0)); // read 1 halfs at a time
        dec[5 * j + 1] = __ldca(((half*)HIGGS_5_256 + 5 * enc[j] + 1)); // read 1 halfs at a time
        dec[5 * j + 2] = __ldca(((half*)HIGGS_5_256 + 5 * enc[j] + 2)); // read 1 halfs at a time
        dec[5 * j + 3] = __ldca(((half*)HIGGS_5_256 + 5 * enc[j] + 3)); // read 1 halfs at a time
        dec[5 * j + 4] = __ldca(((half*)HIGGS_5_256 + 5 * enc[j] + 4)); // read 1 halfs at a time
      }
...
void  higgs_5x256_matvec_cuda(
  ...
  int shared = 16 * 32 * (scales_size / 32 / 8 + 1);
  cudaFuncSetAttribute(
    Higgs5x256dMatVec, cudaFuncAttributeMaxDynamicSharedMemorySize, shared
  );
  Higgs5x256dMatVec<<<blocks, threads, shared>>>(
    (const uint4*) A,
    (const uint4*) B,
    (uint4*) C,
    (const half*) scales,
